...
namespace quadprogpp {
    ...
    public:
        ...
        Vector(const std::vector<T>& a, const unsigned int n);//Initialize by std::vector
        Vector(const Eigen::VectorXd& a,const unsigned int n);
        Vector(const Eigen::VectorXd& a);
        ...
        inline Eigen::VectorXd transform(Vector<T> &a);
    ...
    template<typename T>
    Vector<T>::Vector(const std::vector<T>& a, const unsigned int n)
        :v(new T[2 * n])
    {
        this->n = n;
        for (unsigned int i = 0; i < n; i++)
    ...
    template<typename T>
    Vector<T>::Vector(const Eigen::VectorXd& a,const unsigned int n)
         :v(new T[n])
    {
        this->n=n;
        for(unsigned int i=0;i<n;i++)
    ...
    template<typename T>
    Vector<T>::Vector(const Eigen::VectorXd& a)
            :v(new T[a.size()])
    {
    ...
    template<typename T>
    Eigen::VectorXd transform(Vector<T> &a)
    {
        const int n=a.size();
        Eigen::VectorXd res;
        res.resize(n);
...
#include <stdexcept>
    ...
    */
    ...
    public:
        ...
        Matrix(Eigen::MatrixXd &a,const unsigned int n,const unsigned int m);
        Matrix(Eigen::MatrixXd &a);
    ...
    template<class T>
    Matrix<T>::Matrix(std::vector<std::vector<T> >& a, const unsigned int n, const unsigned int m)
        :v(new T* [2 * n])
    {
        this->n = n;
        this->m = m;
    ...
    template <typename T>
    Matrix<T>::Matrix(Eigen::MatrixXd &a,const unsigned int n,const unsigned int m)
         : v(new T* [n])
    {
             this->n=n;this->m=m;
             v[0]=new T[m*n];
             ...
             for(unsigned int i=0;i<n;i++)
             {
                 for(unsigned int j=0;j<m;j++)
                 {
                     v[i][j]=static_cast<T> (a(i,j));
                 }
             }
    }

    template <typename T>
    Matrix<T>::Matrix(Eigen::MatrixXd &a)
            : v(new T* [a.col(0).size()])
    {
        const unsigned int n=a.col(0).size();
        const unsigned int m=a.row(0).size();
        this->n=n;this->m=m;
        v[0]=new T[m*n];
        for (unsigned int i = 1; i < n; i++)
            v[i] = v[i - 1] + m;
